...
lipsum = {
    ...
    getWord: function () {
        return this.lang[Math.floor(Math.random() * this.lang.length)];
    },

    getEnding: function () {
        var i = Math.floor(Math.random() * this.endings.length);
        return this.endings.substring(i, i + 1);
    },

    /* inserts a number of lorem words. Does not append a space at the end. */
    words: function (numWords) {
        var data = '';
    ...
    sentence: function (numWords) {
        var data = '';
        data += (this.capitalize(this.getWord()) + " ");
        data += this.words(numWords - 1);
        data += (this.getEnding());
        data += (" ");
        return data;
    },
    ...
    sentence2: function (numWords) {
        ...
        if (chance(50)) {
            ...
            data += this.words(part1);
            data += (", ");
        } else {
            data += (" ");
        }
        data += this.words(numWords - part1 - 1);
        data += (this.getEnding());
        data += (" ");
        return data;
    },
    ...
    paragraph: function (numWords) {
        var data = '';
        var words = numWords;
        while (words > 0) {
            if (words > 10) {
                w = Math.floor(Math.random() * 8) + 2;
                data += this.sentence2(w);
                words = words - w;
            } else {
                data += this.sentence(words);
                words = 0;
            }
        }
        return data;
    ...
    numberWithZeroFreq: function(max,freq){
        nbr = Math.floor(Math.random() * (max + 1))
        return (Math.random() < freq) ? 0 : nbr;
    }
};
